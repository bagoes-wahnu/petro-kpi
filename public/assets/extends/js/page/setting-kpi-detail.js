...
jQuery(document).ready(function($) {
    ...
    //     unit_id=localStorage.getItem("userID")
    //     show()
    ...
    $(".checkbox-setting").change(function() {
        if($('.checkbox-setting').is(':checked')) {
...
function selectAll() {
    let stat = $("#checkbox-kpi").prop("checked");
    $(".checkbox-setting").prop("checked", stat);
    if ($(".checkbox-setting").prop("checked")) {
        $("#button-footer").removeClass("d-none");
    }else{
        $("#button-footer").addClass("d-none");
    }
...
function setting(param){
    if (param == 1) {
        $('#setting-perspektif').removeClass('d-none')
        $('#setting-kpi').addClass('d-none')
        $('#btn-dttb-1').addClass('active')
        $('#btn-dttb-2').removeClass('active')
    } else if (param == 2){
        $('#setting-perspektif').addClass('d-none')
        $('#setting-kpi').removeClass('d-none')
        ...
        try {
            ...
            if(data){
                ...
                $.ajax({
                    ...
                    url: urlApi + "setting-kpi/targets?year_id="+id+"&unit_id="+unit_id,
                        ...
                        if(res.length>0){
                            ...
                            if(res.length>0){
                                for (i in res){
                                    ...
                                    if(res[i].parent_id==0){
                                        ...
                                        let chk_disabled=rl_now==rl_direksi||rl_now==rl_admin?"":"d-none"
                                        let div_disabled=rl_now==rl_direksi||rl_now==rl_admin?"col-11":"col-12"
                            ...
                            for(pa in parentArr){
                                ...
                                for (c in chk){
                                    ...
                                    $('#modal-'+parentArr[pa].parent_id+" #target-sub-"+chk[c].id).val(chk[c].target)
                                    $('#modal-'+parentArr[pa].parent_id+" #bobot-sub-"+chk[c].id).val(chk[c].bobot)
...
function fully(){
    $('#modal #pilih-menu').addClass("d-none")
    $('#modal #divisi').removeClass("d-none")
    $('#modal #fully').addClass("active")
    $('#modal #partial').removeClass("active")
    $('#modal #contributing').removeClass("active")
    if ($('#modal #fully').hasClass("active")) {
        console.log("fully")
    }
}
...
function partial(){
    $('#modal #pilih-menu').addClass("d-none")
    $('#modal #divisi').removeClass("d-none")
    $('#modal #fully').removeClass("active")
    $('#modal #partial').addClass("active")
    $('#modal #contributing').removeClass("active")
    if ($('#modal #partial').hasClass("active")) {
        console.log("partial")
    }
}
...
function contributing(){
    ...
    $('#modal #fully').removeClass("active")
    $('#modal #partial').removeClass("active")
    $('#modal #contributing').addClass("active")
    if ($('#modal #contributing').hasClass("active")) {
...
function modalUnitOpen(id_target){
    $('#modal').modal('show')
    $('#modal #pilih-menu').removeClass("d-none")
    $('#modal #divisi').addClass("d-none")
...
function modalReturned(id_target){
    $('#modal-returned').modal('show')
}

function modalHistory(id_target){
...
function tambahPerspektif(){
    ...
    <div class="col-12 row mx-0 p-4 my-8 border-dashed rounded div-parameter new-target" id="parent-`+uuid+`">
        ...
        <div class="col-3 mb-10 ps-0">
            ...
            <select class="form-select select_parameter" data-control="select2" name="lbl-parameter" id="parameter-`+uuid+`" data-placeholder="Pilih perspektif">
            </select>
        </div>
...
function tambahIndikator(){
    ...
    z = x++;    // x is now 3, y is 2
    const html = `
    <div class="d-flex col-11 px-0 py-0 m-0" id="parent-`+uuid+`">
        <h1 id="counter" class="fw-bold me-5 mt-20">#`+z+`</h1>
        <div class="col-12 row mx-0 p-4 my-8 border-dashed rounded div-parameter new-target">
            <input type="hidden" name="targets[]" value="`+uuid+`"/>
